...
import { timeShow } from '../assets/text_processing.js';
var url = window.location.href; // 获取当前 URL
var pathParts = url.split('/'); // 分割路径
var bid = pathParts[4];
var parent_bcid = 0;
var if_comment_sender_open = 0;
function ottohub_load_comment(){
    $('#comment_btn').click(function() {
        if (parent_bcid == 0) {$('#comment_title').text('评论OB'+bid);}
        else {$('#comment_title').text('回复评论OBC'+parent_bcid);}
        if (if_comment_sender_open == 0) {$('#comment_sender').show();if_comment_sender_open = 1;}
        else {$('#comment_sender').hide();if_comment_sender_open = 0;parent_bcid = 0;}
    });
...
function comment_blog() {
    ...
    $.ajax({
        url: 'https://api.ottohub.cn/?module=comment&action=comment_blog&bid='+bid+'&parent_bcid='+parent_bcid+'&content='+content+'&token='+token,  // 请求的 URL
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.status == "success") {
                $('.comment_container[name="'+parent_bcid+'"]').html('');
                $('.comment_container[name="'+parent_bcid+'"]').attr('offset', 0);
                load_comment(parent_bcid);
                $('#comment_content').val('');$('#comment_sender').hide();parent_bcid = 0;
                if (response.if_get_experience == 1) {alert('经验加三');}
                else {alert('今日经验已达上限');}
            } else {
                if (response.message == 'error_bid') {alert('评论棍母');}
                else if (response.message == 'error_parent_bcid') {alert('评论棍母');}
...
function load_comment(parent_comment) {
    ...
    var api = 'https://api.ottohub.cn/?module=comment&action=blog_comment_list&num=12&bid='+bid+'&parent_bcid='+parent_comment+'&offset='+temp_offset;
    var token = getCookie("token");
    if (token != "error") {api = 'https://api.ottohub.cn/?module=comment&action=blog_comment_list&num=12&bid='+bid+'&parent_bcid='+parent_comment+'&offset='+temp_offset+'&token='+token;}
    $.ajax({
        url: api,  // 请求的 URL
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.status == "success") {
                ...
                comment_list.forEach(function(comment) {
                    comment_html = comment_html + comment_card(comment.bcid, comment.parent_bcid, comment.uid, comment.content, comment.time, comment.child_comment_num, comment.if_my_comment, comment.username, comment.honour, comment.avatar_url);
                });
                ...
                $('.comment_btn').click(function() {
                    ...
                    if (parent_cid == 0) {parent_bcid = name;}
                    else {parent_bcid = parent_cid;$('#comment_content').val('@'+username+' '+$('#comment_content').val());}
                    $('#comment_title').text('回复评论OBC'+parent_bcid);
                    $('#comment_sender').show();
...
function report_comment(cid) {
    ...
    if (token == "error") {alert('那我缺的登录这块谁来给我补啊');}
    $.ajax({
        url: 'https://api.ottohub.cn/?module=comment&action=report_blog_comment&bcid='+cid+'&token='+token,  // 请求的 URL
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.status == "success") {
                ...
                if (response.message == 'error_bcid') {alert('举报棍母');}
                else if (response.message == 'error_token') {alert('那我缺的登录这块谁来给我补啊');}
...
function delete_comment(cid) {
    ...
    if (token == "error") {alert('那我缺的登录这块谁来给我补啊');}
    $.ajax({
        url: 'https://api.ottohub.cn/?module=comment&action=delete_blog_comment&bcid='+cid+'&token='+token,  // 请求的 URL
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.status == "success") {
                ...
                if (response.message == 'error_bcid') {alert('删除棍母');}
                else if (response.message == 'error_token') {alert('那我缺的登录这块谁来给我补啊');}
...
<div class="comment_subtitle black-text">${timeShow(time)} OBC${cid}</div>
</div>
